...
// allows for any custom implementation. From a json for example.
type actionGetter interface {
	getActions() ([]Action, error)
}

func _main() error {
	ag := &astActionGetter{fileName: "api.go"}
	actions, err := ag.getActions()
	if err != nil {
		return err
	}
...
func (a *astActionGetter) getActions() ([]Action, error) {
	fs := token.NewFileSet()
	p, err := parser.ParseFile(fs, a.fileName, nil, parser.AllErrors)
	if err != nil {
		return nil, err
	}
...
func writeFunction(buf *bytes.Buffer, method Method) {
	...
	buf.WriteString("{\n")
	writeGetMethod(buf, method)

	buf.WriteString("}\n\n")
}

func writeGetMethod(buf *bytes.Buffer, method Method) {
	if len(method.outputParams) > 1 {
		buf.WriteString("var output = &")
		buf.WriteString(strings.TrimLeft(method.outputParams[0].Type, "*"))
		buf.WriteString("{}")
	}

	buf.WriteString("\nop := client.Operation{")
	buf.WriteString("\nHTTPPath: r.baseURL + v1 + \"" + method.endpoint + "\",")
